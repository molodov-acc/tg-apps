...
if (!botToken) throw new Error("BOT_TOKEN is required for Telegram initData verification");
...
app.post("/auth/telegram", async (req, reply) => {
  ...
  if (!data.user) return reply.code(401).send({ error: "No user in initData" });

  const userObj = JSON.parse(data.user) as {
    id: number;
    username?: string;
    first_name?: string;
    last_name?: string;
...
app.post("/attempts", async (req, reply) => {
  ...
  const data = verifyTelegramWebAppInitData(parsed.data.initData, botToken);
  if (!data || !data.user) return reply.code(401).send({ error: "Invalid initData" });

  const userObj = JSON.parse(data.user) as { id: number };
  const dbUser = await prisma.user.findUnique({ where: { telegramId: String(userObj.id) } });
  if (!dbUser) return reply.code(401).send({ error: "User not found; call /auth/telegram first" });
...
app.post("/bookmarks/:questionId", async (req, reply) => {
  const questionId = (req.params as any).questionId as string;
